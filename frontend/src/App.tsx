...
const App: React.FC = () => {
  ...
  const handleResult = useCallback(
    (title: string, items: ScrapedItem[] | null) => {
      if (items) {
        setAllResults((prev) => {
  ...
  return (
    <div className="container">
      ...
      <div
        ...
        style={{ display: view === "instagram" ? "none" : "block" }}
      >
        <h1>Party Scraper</h1>
        ...
        >
          <div style={{ position: "relative" }}>
            ...
            >
              {isAnyLoading
                ? `FETCHING... (${activeScrapersCount}/${VENUES.length})`
          ...
          {aggregatedResults.length > 0 && (
            <div style={{ display: "flex", gap: "1rem" }}>
              ...
              >
                {copiedAll
                  ? "Copied All (JSON)!"
                  : `Copy All (${aggregatedResults.length})`}
              </button>
            </div>
          )}
        </div>
      ...
      <div
        ...
        style={{ display: view === "instagram" ? "block" : "none" }}
      >
